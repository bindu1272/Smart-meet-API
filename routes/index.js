const ApiRoutesV1 = require('./v1/api-v1');
...
function processRoutes(route, globalMiddlewares = []) {
  ...
  for (let routeKey in route) {
    let middlewareArr = [];
    let routeMiddlewareArr = App.lodash.uniq(
    ...
    for (let middlewareIndex in routeMiddlewareArr) {
      let middleware = getComputedMiddleware(
        App.helpers.getObjProp(routeMiddlewareArr, middlewareIndex)
      );
    ...
    if (routeKey === 'POST menu-categories') {
    }
